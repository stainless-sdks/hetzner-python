...
class TestActions:
    ...
    def test_method_retrieve(self, client: Hetzner) -> None:
        action = client.certificates.actions.retrieve(
            0,
            id=0,
        )
    ...
    def test_method_list(self, client: Hetzner) -> None:
        action = client.certificates.actions.list(
            0,
        )
    ...
    @parametrize
    def test_method_retry(self, client: Hetzner) -> None:
        action = client.certificates.actions.retry(
            0,
        )
        assert_matches_type(ActionRetryResponse, action, path=["response"])
...
class TestAsyncActions:
    ...
    async def test_method_retrieve(self, client: AsyncHetzner) -> None:
        action = await client.certificates.actions.retrieve(
            0,
            id=0,
        )
        assert_matches_type(ActionRetrieveResponse, action, path=["response"])
    ...
    async def test_method_list(self, client: AsyncHetzner) -> None:
        action = await client.certificates.actions.list(
            0,
        )
        assert_matches_type(ActionListResponse, action, path=["response"])
    ...
    @parametrize
    async def test_method_retry(self, client: AsyncHetzner) -> None:
        action = await client.certificates.actions.retry(
            0,
        )
        assert_matches_type(ActionRetryResponse, action, path=["response"])
