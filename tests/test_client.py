...
from hetzner._models import BaseModel, FinalRequestOptions
from hetzner._base_client import BaseClient, make_request_options
...
class TestHetzner:
    client = Hetzner(base_url=base_url, api_token=api_token, _strict_response_validation=True)

    def test_raw_response(self) -> None:
        response = self.client.get("/actions", options={"params": {}}, cast_to=httpx.Response)
    ...
    def test_copy(self) -> None:
        ...
        copied = self.client.copy(api_token="my new api token")
        assert copied.api_token == "my new api token"
        assert self.client.api_token == api_token
...
class TestAsyncHetzner:
    client = AsyncHetzner(base_url=base_url, api_token=api_token, _strict_response_validation=True)

    async def test_raw_response(self) -> None:
        response = await self.client.get("/actions", options={"params": {}}, cast_to=httpx.Response)
        assert response.status_code == 200
        assert isinstance(response, httpx.Response)
